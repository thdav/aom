  ...
  while (more_data) {
    ...
    comb_token = last_pos ? 2 * av1_read_record_bit(xd->counts, r, ACCT_STR) + 2
                          : av1_read_record_symbol(
                                ...
                                !first_pos;
    if (xd->counts && !last_pos) coef_head_counts[band][ctx][comb_token - !first_pos]++;
    if (first_pos) {
      ...
      comb_token =
          last_pos
              ...
              : av1_read_record_symbol(xd->counts, r, coef_head_cdfs[band][ctx],
                    ...
                    1;
      token = comb_token >> 1;
      if (xd->counts && !last_pos) coef_head_counts[band][ctx][comb_token - 1]++;
    }

    more_data = comb_token & 1;

    if (token > ONE_TOKEN) {
      token += av1_read_record_symbol(xd->counts, r, coef_tail_cdfs[band][ctx > 1 ? 2: 0],
                                      TAIL_TOKENS, ACCT_STR);
      if (xd->counts) coef_tail_counts[band][ctx> 1 ? 2:0][token - TWO_TOKEN]++;
    }
#if CONFIG_NEW_QUANT
    dqv_val = &dq_val[band][0];
#endif  // CONFIG_NEW_QUANT
