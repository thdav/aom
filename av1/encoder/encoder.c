...
static void make_update_tile_list_enc(AV1_COMP* cpi, const int tile_rows, const int tile_cols, FRAME_CONTEXT *ec_ctxs[])
{
  int i;
...
// off.
  ...
  if (cm->refresh_frame_context == REFRESH_FRAME_CONTEXT_BACKWARD) {
    ...
    av1_average_tile_coef_cdfs(cpi->common.fc, tile_ctxs, cm->tile_rows * cm->tile_cols);
#endif
    av1_adapt_intra_frame_probs(cm);
#if CONFIG_EC_ADAPT
    av1_average_tile_intra_cdfs(cpi->common.fc, tile_ctxs, cm->tile_rows * cm->tile_cols);
#endif
  }
...
#if CONFIG_EC_ADAPT
      av1_average_tile_inter_cdfs(&cpi->common, cpi->common.fc, tile_ctxs, cm->tile_rows * cm->tile_cols);
#endif
      av1_adapt_mv_probs(cm, cm->allow_high_precision_mv);
#if CONFIG_EC_ADAPT
      av1_average_tile_mv_cdfs(cpi->common.fc, tile_ctxs, cm->tile_rows * cm->tile_cols);
#endif
    }
  }
