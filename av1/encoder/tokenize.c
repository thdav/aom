...
static void tokenize_b(int plane, int block, int blk_row, int blk_col,
  ...
  if (eob == 0)
    add_token(&t, &coef_tail_cdfs[band[c]][pt*!CONFIG_COEFF_CTX_REDUCE], &coef_head_cdfs[band[c]][pt], 1,
              1, 0, BLOCK_Z_TOKEN);
  ...
  while (c < eob) {
    ...
    if (!v) {
      add_token(&t, &coef_tail_cdfs[band[c]][pt*!CONFIG_COEFF_CTX_REDUCE], &coef_head_cdfs[band[c]][pt],
