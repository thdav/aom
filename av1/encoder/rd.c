...
static void set_prob_tokens(int prob_head[ENTROPY_TOKENS],
  ...
  int prob_tmp[5];
  int prob_block_nz = CDF_PROB_TOP - cdf_head[0];
  int scale;
  int r;

  prob_tmp[ZERO_TOKEN] =
      cdf_head[1 + ZERO_TOKEN] - cdf_head[1 + ZERO_TOKEN - 1];
  prob_tmp[ONE_TOKEN_EOB] =
      cdf_head[1 + ONE_TOKEN_EOB] - cdf_head[1 + ONE_TOKEN_EOB - 1];
  prob_tmp[ONE_TOKEN_NEOB] =
      cdf_head[1 + ONE_TOKEN_NEOB] - cdf_head[1 + ONE_TOKEN_NEOB - 1];
  prob_tmp[TWO_TOKEN_PLUS_EOB] =
      cdf_head[1 + TWO_TOKEN_PLUS_EOB] - cdf_head[1 + TWO_TOKEN_PLUS_EOB - 1];
